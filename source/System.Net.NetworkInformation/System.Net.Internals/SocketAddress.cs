...
internal class SocketAddress
{
	...
	internal IPAddress GetIPAddress()
	{
		...
		{
			Span<byte> span = stackalloc byte[16];
			System.Net.SocketAddressPal.GetIPv6Address(Buffer, span, out var scope);
			return new IPAddress(span, scope);
		}
		if (Family == AddressFamily.InterNetwork)
		{
			long newAddress = (long)System.Net.SocketAddressPal.GetIPv4Address(Buffer) & 0xFFFFFFFFL;
	...
	{
		if (!(comparand is System.Net.Internals.SocketAddress socketAddress) || Size != socketAddress.Size)
		{
			return false;
		}
