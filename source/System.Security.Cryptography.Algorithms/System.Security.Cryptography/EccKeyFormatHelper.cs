...
{
	internal unsafe static ECParameters FromECPrivateKey(ReadOnlySpan<byte> key, out int bytesRead)
	{
		try
		{
			AsnDecoder.ReadEncodedValue(key, AsnEncodingRules.BER, out var _, out var _, out var bytesConsumed);
			fixed (byte* pointer = &MemoryMarshal.GetReference(key))
			{
				...
				FromECPrivateKey(memoryManager.Memory, in algId, out var ret);
	...
	internal static void FromECPrivateKey(ReadOnlyMemory<byte> keyData, in AlgorithmIdentifierAsn algId, out ECParameters ret)
	{
		ECPrivateKey key = ECPrivateKey.Decode(keyData, AsnEncodingRules.BER);
		FromECPrivateKey(key, in algId, out ret);
	}

	internal static void FromECPrivateKey(ECPrivateKey key, in AlgorithmIdentifierAsn algId, out ECParameters ret)
	{
		...
		ECDomainParameters domainParameters = ((!key.Parameters.HasValue) ? ECDomainParameters.Decode(algId.Parameters.Value, AsnEncodingRules.DER) : key.Parameters.Value);
	...
	internal static void FromECPublicKey(ReadOnlyMemory<byte> key, in AlgorithmIdentifierAsn algId, out ECParameters ret)
	{
		if (!algId.Parameters.HasValue)
		{
			throw new CryptographicException(System.SR.Cryptography_Der_Invalid_Encoding);
		}
		ReadOnlySpan<byte> span = key.Span;
		if (span.Length == 0)
		{
			throw new CryptographicException(System.SR.Cryptography_Der_Invalid_Encoding);
		}
		...
		int num = span.Length / 2;
		ECDomainParameters domainParameters = ECDomainParameters.Decode(algId.Parameters.Value, AsnEncodingRules.DER);
		ret = new ECParameters
		{
			Curve = GetCurve(domainParameters),
			Q = 
			{
				X = span.Slice(1, num).ToArray(),
				Y = span.Slice(1 + num).ToArray()
			}
		};
		ret.Validate();
	}
	...
	private static ECCurve GetCurve(ECDomainParameters domainParameters)
	{
		...
		{
			"1.2.840.10045.3.1.7" => Oids.secp256r1Oid, 
			"1.3.132.0.34" => Oids.secp384r1Oid, 
			"1.3.132.0.35" => Oids.secp521r1Oid, 
	...
	internal static AsnWriter WritePkcs8PrivateKey(ECParameters ecParameters, AttributeAsn[] attributes = null)
	{
		...
		return KeyFormatHelper.WritePkcs8(algorithmIdentifierWriter, privateKeyWriter, attributesWriter);
	}
	...
	{
		return WriteEcPrivateKey(in ecParameters, includeDomainParameters: true);
	}
