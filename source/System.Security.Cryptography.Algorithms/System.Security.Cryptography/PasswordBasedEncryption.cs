...
internal static class PasswordBasedEncryption
{
	...
	internal unsafe static int Decrypt(in AlgorithmIdentifierAsn algorithmIdentifier, ReadOnlySpan<char> password, ReadOnlySpan<byte> passwordBytes, ReadOnlySpan<byte> encryptedData, Span<byte> destination)
	{
	...
	internal unsafe static int Encrypt(ReadOnlySpan<char> password, ReadOnlySpan<byte> passwordBytes, SymmetricAlgorithm cipher, bool isPkcs12, AsnWriter source, PbeParameters pbeParameters, ReadOnlySpan<byte> salt, byte[] destination, Span<byte> ivDest)
	{
		...
		fixed (byte* ptr3 = array2)
		{
			fixed (byte* ptr2 = array)
			{
				...
				{
					array3 = new byte[num];
					Pkcs12Kdf.DeriveCipherKey(password, hashAlgorithm, iterationCount, salt, array3);
					Pkcs12Kdf.DeriveIV(password, hashAlgorithm, iterationCount, salt, iV);
					ivDest.Clear();
				}
				else
				{
