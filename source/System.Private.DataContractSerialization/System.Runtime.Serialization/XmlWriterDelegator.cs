...
internal class XmlWriterDelegator
{
	...
	internal void WriteXmlnsAttribute(string ns)
	{
		...
		if (text == null)
		{
			...
			handler.AppendFormatted(depth);
			handler.AppendLiteral("p");
			handler.AppendFormatted(_prefixes);
	...
	{
		return new InvalidDataContractException(System.SR.Format(System.SR.InvalidPrimitiveType_Serialization, DataContract.GetClrTypeFullName(type)));
	}
