...
internal sealed class NTAuthentication
{
	...
	{
		return System.Net.Security.NegotiateStreamPal.VerifySignature(_securityContext, buffer, offset, count);
	}

	internal int MakeSignature(byte[] buffer, int offset, int count, [AllowNull] ref byte[] output)
	{
		return System.Net.Security.NegotiateStreamPal.MakeSignature(_securityContext, buffer, offset, count, ref output);
	}
