...
internal static class SSPIWrapper
{
	...
	private unsafe static int EncryptDecryptHelper(OP op, ISSPIInterface secModule, SafeDeleteContext context, Span<SecurityBuffer> input, uint sequenceNumber)
	{
			...
			fixed (byte* ptr4 = ((input.Length > 0) ? input[0].token : null))
			{
				fixed (byte* ptr3 = ((input.Length > 1) ? input[1].token : null))
				{
					fixed (byte* ptr2 = ((input.Length > 2) ? input[2].token : null))
					{
						...
						{
							ref SecurityBuffer reference = ref input[i];
						...
						{
							ref SecurityBuffer reference2 = ref input[j];
	...
	public static string QueryStringContextAttributes(ISSPIInterface secModule, SafeDeleteContext securityContext, global::Interop.SspiCli.ContextAttribute contextAttribute)
	{
		...
		SafeHandle refHandle;
		int num = secModule.QueryContextAttributes(securityContext, contextAttribute, MemoryMarshal.AsBytes(span), typeof(SafeFreeContextBuffer), out refHandle);
		using (refHandle)
		{
			if (num != 0)
			{
	...
	public static SafeFreeCertContext QueryContextAttributes_SECPKG_ATTR_REMOTE_CERT_CONTEXT(ISSPIInterface secModule, SafeDeleteContext securityContext)
	{
		Span<IntPtr> span = stackalloc IntPtr[1];
		SafeHandle refHandle;
		int num = secModule.QueryContextAttributes(securityContext, global::Interop.SspiCli.ContextAttribute.SECPKG_ATTR_REMOTE_CERT_CONTEXT, MemoryMarshal.AsBytes(span), typeof(SafeFreeCertContext), out refHandle);
		if (num != 0)
		{
			refHandle?.Dispose();
			if (System.Net.NetEventSource.Log.IsEnabled())
			{
				System.Net.NetEventSource.Error(null, $"ERROR = {ErrorDescription(num)}", "QueryContextAttributes_SECPKG_ATTR_REMOTE_CERT_CONTEXT");
			}
			return null;
		}
		return (SafeFreeCertContext)refHandle;
	}
	...
	{
		Span<global::Interop.SspiCli.SecPkgContext_IssuerListInfoEx> span = MemoryMarshal.CreateSpan(ref ctx, 1);
