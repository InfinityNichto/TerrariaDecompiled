...
internal static class CngPkcs8
{
	...
	internal unsafe static Pkcs8Response ImportEncryptedPkcs8PrivateKey(ReadOnlySpan<byte> passwordBytes, ReadOnlySpan<byte> source, out int bytesRead)
	{
		fixed (byte* pointer = &MemoryMarshal.GetReference(source))
		{
			...
			try
			{
				ArraySegment<byte> arraySegment = System.Security.Cryptography.KeyFormatHelper.DecryptPkcs8(passwordBytes, memoryManager.Memory, out bytesRead);
	...
	internal unsafe static Pkcs8Response ImportEncryptedPkcs8PrivateKey(ReadOnlySpan<char> password, ReadOnlySpan<byte> source, out int bytesRead)
	{
		try
		{
			...
			fixed (byte* pointer = &MemoryMarshal.GetReference(source))
			{
				...
				int bytesRead2;
				ArraySegment<byte> arraySegment = System.Security.Cryptography.KeyFormatHelper.DecryptPkcs8(password, memoryManager.Memory.Slice(0, bytesConsumed), out bytesRead2);
				Span<byte> span = arraySegment;
				try
				{
	...
	private static AsnWriter RewriteEncryptedPkcs8PrivateKey(AsymmetricAlgorithm key, ReadOnlySpan<byte> passwordBytes, PbeParameters pbeParameters)
	{
		...
		try
		{
			...
			return System.Security.Cryptography.KeyFormatHelper.ReencryptPkcs8(span, array.AsMemory(0, bytesWritten), passwordBytes, pbeParameters);
		}
		finally
		{
			span.Clear();
	...
	private static AsnWriter RewriteEncryptedPkcs8PrivateKey(AsymmetricAlgorithm key, ReadOnlySpan<char> password, PbeParameters pbeParameters)
	{
		...
		try
		{
			...
			return System.Security.Cryptography.KeyFormatHelper.ReencryptPkcs8(password, array.AsMemory(0, bytesWritten), password, pbeParameters);
		}
		finally
		{
			System.Security.Cryptography.CryptoPool.Return(array, bytesWritten);
	...
	private unsafe static AsnWriter RewritePkcs8ECPrivateKeyWithZeroPublicKey(ReadOnlySpan<byte> source)
	{
		...
		{
			using MemoryManager<byte> memoryManager = new System.Buffers.PointerMemoryManager<byte>(pointer, source.Length);
			System.Security.Cryptography.Asn1.PrivateKeyInfoAsn privateKeyInfoAsn = System.Security.Cryptography.Asn1.PrivateKeyInfoAsn.Decode(memoryManager.Memory, AsnEncodingRules.BER);
			System.Security.Cryptography.Asn1.AlgorithmIdentifierAsn algId = privateKeyInfoAsn.PrivateKeyAlgorithm;
			if (algId.Algorithm != "1.2.840.10045.2.1")
			{
				return null;
			}
			System.Security.Cryptography.Asn1.ECPrivateKey key = System.Security.Cryptography.Asn1.ECPrivateKey.Decode(privateKeyInfoAsn.PrivateKey, AsnEncodingRules.BER);
			System.Security.Cryptography.EccKeyFormatHelper.FromECPrivateKey(key, in algId, out var ret);
			fixed (byte* ptr = ret.D)
			{
				try
				{
					...
					ret.Q.Y = array;
					ret.Q.X = array;
					return System.Security.Cryptography.EccKeyFormatHelper.WritePkcs8PrivateKey(ret, privateKeyInfoAsn.Attributes);
				}
				finally
				{
					Array.Clear(ret.D);
